    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            if (this.mRunning) {
                ...
                float f3 = (((jCurrentAnimationTimeMillis - this.mFinishTime) % 1000) / 10.0f) / 100.0f;
                float f4 = i;
                float interpolation = INTERPOLATOR.getInterpolation(f3) * f4;
                this.mClipRect.set(f4 - interpolation, 0.0f, f4 + interpolation, iHeight);
                canvas.saveLayerAlpha(this.mClipRect, 0, 0);
                z = true;
            }
            if (j2 != 0) {
                int i3 = (f < 0.0f || f >= 25.0f) ? (f < 25.0f || f >= 50.0f) ? (f < 50.0f || f >= 75.0f) ? this.mColor3 : this.mColor2 : this.mColor1 : this.mColor4;
                canvas.drawColor(i3);
                if (f >= 0.0f && f <= 25.0f) {
