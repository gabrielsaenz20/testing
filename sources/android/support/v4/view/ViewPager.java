...
public class ViewPager extends ViewGroup {
    ...
    private boolean performDrag(float f) {
        ...
        if (scrollX < f3) {
            zOnPull = z ? this.mLeftEdge.onPull(Math.abs(f3 - scrollX) / clientWidth) : false;
            scrollX = f3;
        } else if (scrollX > f4) {
            zOnPull = z2 ? this.mRightEdge.onPull(Math.abs(scrollX - f4) / clientWidth) : false;
            scrollX = f4;
        }
        int i = (int) scrollX;
    ...
    private boolean resetTouch() {
        this.mActivePointerId = -1;
        endDrag();
        return this.mRightEdge.onRelease() | this.mLeftEdge.onRelease();
    }
    ...
    */
    public boolean arrowScroll(int i) throws Resources.NotFoundException {
        ...
        if (viewFindNextFocus == null || viewFindNextFocus == view) {
            ...
            zPageRight = (view == null || getChildRectInPagerCoordinates(this.mTempRect, viewFindNextFocus).left < getChildRectInPagerCoordinates(this.mTempRect, view).left) ? viewFindNextFocus.requestFocus() : pageLeft();
        ...
        for (int i8 = 0; i8 < childCount; i8++) {
            ...
            if (childAt.getVisibility() != 8) {
                ...
                if (layoutParams.isDecor) {
                    ...
                    int i10 = layoutParams.gravity & android.support.design.R.styleable.AppCompatTheme_windowMinWidthMajor;
                    if (i9 == 1) {
    ...
    protected void onMeasure(int i, int i2) throws Resources.NotFoundException {
        ...
        while (true) {
            ...
            if (childAt.getVisibility() != 8 && (layoutParams2 = (LayoutParams) childAt.getLayoutParams()) != null && layoutParams2.isDecor) {
                ...
                int i9 = layoutParams2.gravity & android.support.design.R.styleable.AppCompatTheme_windowMinWidthMajor;
    ...
    */
    void populate(int i) throws Resources.NotFoundException {
        ...
        while (true) {
            ...
            itemInfoAddNewItem = this.mItems.get(i3);
        ...
        if (itemInfoAddNewItem == null && count > 0) {
            itemInfoAddNewItem = addNewItem(this.mCurItem, i3);
        }
    ...
    public void setPageTransformer(boolean z, PageTransformer pageTransformer, int i) throws IllegalAccessException, Resources.NotFoundException, IllegalArgumentException, InvocationTargetException {
        if (Build.VERSION.SDK_INT >= 11) {
    ...
    void smoothScrollTo(int i, int i2, int i3) throws Resources.NotFoundException {
        ...
        int iMin = Math.min(iAbs > 0 ? 4 * Math.round(1000.0f * Math.abs(fDistanceInfluenceForSnapDuration / iAbs)) : (int) (((Math.abs(i5) / ((f * this.mAdapter.getPageWidth(this.mCurItem)) + this.mPageMargin)) + 1.0f) * 100.0f), MAX_SETTLE_DURATION);
