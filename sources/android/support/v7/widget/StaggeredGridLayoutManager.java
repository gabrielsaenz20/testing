...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        int i4 = this.mLayoutState.mInfinite ? layoutState.mLayoutDirection == 1 ? ActivityChooserView.ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED : Integer.MIN_VALUE : layoutState.mLayoutDirection == 1 ? layoutState.mEndLine + layoutState.mAvailable : layoutState.mStartLine - layoutState.mAvailable;
        ...
        while (layoutState.hasMore(state) && (this.mLayoutState.mInfinite || !this.mRemainingSpans.isEmpty())) {
            ...
            layoutManager.layoutDecoratedWithMargins(view, i2, decoratedMeasurement2, i3, i);
            if (layoutParams.mFullSpan) {
        ...
        return startAfterPadding2 > 0 ? Math.min(layoutState.mAvailable, startAfterPadding2) : i5;
    }
    ...
    private void recycle(RecyclerView.Recycler recycler, LayoutState layoutState) {
        ...
        if (layoutState.mAvailable != 0) {
            if (layoutState.mLayoutDirection == -1) {
                ...
                int iMin = maxStart < 0 ? layoutState.mEndLine : layoutState.mEndLine - Math.min(maxStart, layoutState.mAvailable);
                recycleFromEnd(recycler, iMin);
            } else {
                int minEnd = getMinEnd(layoutState.mEndLine) - layoutState.mEndLine;
                int iMin2 = minEnd < 0 ? layoutState.mStartLine : Math.min(minEnd, layoutState.mAvailable) + layoutState.mStartLine;
                recycleFromStart(recycler, iMin2);
            }
        }
    }
    ...
    public void collectAdjacentPrefetchPositions(int i, int i2, RecyclerView.State state, RecyclerView.LayoutManager.LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        for (int i3 = 0; i3 < this.mSpanCount; i3++) {
            ...
            if (this.mLayoutState.mItemDirection == -1) {
                ...
                startLine = this.mSpans[i3].getStartLine(this.mLayoutState.mStartLine);
            } else {
                endLine = this.mSpans[i3].getEndLine(this.mLayoutState.mEndLine);
                startLine = this.mLayoutState.mEndLine;
            }
            iArr[i3] = endLine - startLine;
        }
        Arrays.sort(this.mPrefetchDistances, 0, this.mSpanCount);
        for (int i4 = 0; i4 < this.mSpanCount && this.mLayoutState.hasMore(state); i4++) {
