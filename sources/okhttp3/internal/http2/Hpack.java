    ...
    static final class Reader {
        ...
        private ByteString getName(int i) throws IOException {
            ...
            if (isStaticHeader(i)) {
                ...
                if (iDynamicTableIndex < 0 || iDynamicTableIndex >= this.dynamicTable.length) {
                    throw new IOException("Header index too large " + (i + 1));
                }
                header = this.dynamicTable[iDynamicTableIndex];
            }
            return header.name;
        }
                ...
                if (num != null) {
                    ...
                    if (length == -1) {
                        ...
                        while (true) {
                            ...
                            if (Util.equal(this.dynamicTable[i2].name, asciiLowercase)) {
                                if (Util.equal(this.dynamicTable[i2].value, byteString)) {
                                    length = Hpack.STATIC_HEADER_TABLE.length + (i2 - this.nextHeaderIndex);
                                    break;
                                } else if (length2 == -1) {
                                    length2 = (i2 - this.nextHeaderIndex) + Hpack.STATIC_HEADER_TABLE.length;
                                }
                            }
                            i2++;
                        }
